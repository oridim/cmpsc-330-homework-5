...
void PrintBoard(int iter, const Board &board, const vector<PlayerInfo> &players)
{
    ...
    for (int i = 0; i < players.size(); i++)
    {
        ...
        {
            cout << players[i].score << " boxes";
        }
        else
        {
            cout << players[i].score << " box";
        }
...
string GameMaster(const vector<string> &libpaths, vector<int> &scores, vector<double> &timespents, double sleep_sec)
{
    ...
    while (true)
    {
        ...
        try
        {
            ...
            if (loc.IsBoxLocation())
            {
                throw "lose";
            }
            if (loc.IsDotLocation())
            {
        ...
        if (loc.IsLineHorizontalLocation())
        {
            if (loc.row - 1 >= 0)
            {
                if (AddBox(board, loc.row - 1, loc.col, player->box, players))
                {
                    player->score++;
                    gainbox++;
                }
            }
            if (loc.row + 1 < board.GetRows())
            {
                if (AddBox(board, loc.row + 1, loc.col, player->box, players))
                {
                    player->score++;
        ...
        if (loc.IsLineVerticalLocation())
        {
            if (loc.col - 1 >= 0)
            {
                if (AddBox(board, loc.row, loc.col - 1, player->box, players))
                {
                    player->score++;
                    gainbox++;
                }
            }
            if (loc.col + 1 < board.GetCols())
            {
                if (AddBox(board, loc.row, loc.col + 1, player->box, players))
                {
                    player->score++;
